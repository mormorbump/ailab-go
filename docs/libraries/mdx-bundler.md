...
MDXコンテンツとその依存関係を高速にコンパイル・バンドルするツール。MDX v3とesbuildを使用し、MDXファイル内でのJavaScriptやReactコンポーネントのインポートをサポート。
...
- **SSRでの使用**: `getMDXComponent`はクライアントサイドでのみ使用（サーバーでは`new Function`を使う）
